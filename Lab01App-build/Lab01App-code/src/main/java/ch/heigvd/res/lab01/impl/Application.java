...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    // Here, we create the folders in the filename path
    try {
      File path = new File(filename);
      path.mkdirs();
    } catch (Exception e) {
      System.out.println("Failed to create directories for PATH: " + filename);
    }
    
    // Once the tags are in the filename, we add the name of the output file.
    filename += "/quote-" + quote.getValue().getId() + ".utf8";
  
    // We declare the output stream
    FileWriter output = null;
    
    try {
      // We initialise the output stream.
      output = new FileWriter(filename);
      
      // And write the quote in it.
  ...
  void printFileNames(final Writer writer) {
    ...
    explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
      ...
      public void visit(File file) {
        try {
          // We write the file path to the writer
          writer.write(file.getPath() + "\n");
        } catch (Exception e) {
          System.out.println("Failed to write filename path");
        }
      }
    });
  }
  ...
  public String getAuthorEmail() {
    return "daniel.gonzalezlopez@heig-vd.ch";
  }
