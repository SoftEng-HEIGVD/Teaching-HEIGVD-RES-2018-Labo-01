...
public class Application implements IApplication {
    ...
    /**
     * This method will create the working directory and the quote directory
     *
     * @throws IOException
     */
    ...
    void storeQuote(Quote quote, String filename) throws IOException {
        // Getting tags and text from quote
        List<String> tags = quote.getTags();
        String text = quote.getQuote();
        // Initializing the path
        String path = WORKSPACE_DIRECTORY;

        // Adding all the tags to the path
        for (String tag : tags) {
            path += "/";
            path += tag;
        }

        // Creating directories from path
        new File(path).mkdirs();

        // Updating path with filename
        path += "/";
        path += filename;

        /// Creating file and writing the quote text in it
    ...
    void printFileNames(final Writer writer) {
        ...
        explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
            ...
            public void visit(File file) {
                try {
                    // writing the path of the file in the writer
