...
public class Application implements IApplication {
    ...
    public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException {
        ...
        for (int i = 0; i < numberOfQuotes; i++) {
            ...
            // For each desired quote => store the quote (create the file and path)
            String quoteFileName = "/quote-" + i + ".utf8";
            storeQuote(quote, quoteFileName);
    ...
    void storeQuote(Quote quote, String filename) throws IOException {
        String quoteDirPath = WORKSPACE_DIRECTORY;
        for (String tag : quote.getTags()) {
            quoteDirPath += "/" + tag;
        }

        new File(quoteDirPath).mkdirs();
        File quoteFile = new File(quoteDirPath + filename) ;
        quoteFile.createNewFile();

        BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(quoteFile), "UTF-8"));
        ...
        explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
            ...
            public void visit(File file) {
                ...
                try {
                    ...
                    LOG.log(Level.SEVERE, "Could not print file names", e.getMessage());
                    e.printStackTrace();
                }
