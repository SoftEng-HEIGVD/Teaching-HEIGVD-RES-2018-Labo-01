...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    String where = WORKSPACE_DIRECTORY;
    for (String tag : quote.getTags()) where += File.separator + tag;

    File dir = new File(where);
    dir.mkdirs();

    OutputStreamWriter OSWriter = new OutputStreamWriter(new FileOutputStream(new File(where + File.separator + filename)), "UTF-8");
  ...
  public String getAuthorEmail() {
    return "nathan.fluckiger@heig-vd.ch";
  }
