...
public class Application implements IApplication {
    ...
    /**
     * @author modify by Yann Lederrey
     * Adding information for quote with number of this and extensions utf8.
     */
    @Override
    public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException {
        ...
        for (int i = 0; i < numberOfQuotes; i++) {
            ...
            tempStr.append("quote-").append(i).append(".utf8");
            storeQuote(quote, tempStr.toString());
    ...
    void storeQuote(Quote quote, String filename) throws IOException {
        ...
        File workspaceQuoteDir = new File(WORKSPACE_DIRECTORY);
        File workspaceDir = new File("./workspace");
        StringBuilder myDir = new StringBuilder(WORKSPACE_DIRECTORY + "/");

        List<String> myTags = quote.getTags();
        String myQuote = quote.getQuote();

        // verify if the simple workspace directory not exist
        if (!workspaceDir.exists()) {
            // we create it
            workspaceDir.mkdir();
        }

        // verify if the quote directory not exist
        if (!workspaceQuoteDir.exists())
            // we create it
            workspaceQuoteDir.mkdir();

        // for every Tags we create a new Directory from workspace directory and tag name.
        for (String myTag : myTags) {
            File myNewDir = new File(myDir + myTag);

            // verify if it exist, if not we create it.
            if (!myNewDir.exists())
                myNewDir.mkdir();

            // We add the ususal character '/' for the next depth directory.
            myDir.append(myTag).append("/");
        }

        // we create the quote file from fetchAndStoreQuotes
        // filename is the name of the quote.
        myDir.append(filename);
        File newQuoteFile = new File(myDir.toString());
        newQuoteFile.createNewFile();

        OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(newQuoteFile), "utf8");
        writer.write(myQuote);
        writer.close();
    }
        ...
        explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
            ...
            public void visit(File file) {
                try {
                    ...
                    System.out.println("Cannot get the path file");
                }
            }
        });
    }
