...
public class Application implements IApplication {
	...
	void storeQuote(Quote quote, String filename) throws IOException {
		//it create first the pathname then the sub-folders with the tags and the file
		...
		File text = new File(path + "/quote-" + quoteCounter + ".utf8");
		text.createNewFile();

		//finally it write the quote in the file
		String textFromQuote = quote.getQuote();
		Writer writer = new OutputStreamWriter(new FileOutputStream(path + "/quote-" + quoteCounter++ + ".utf8"));
		writer.write(textFromQuote);
		writer.flush();
		writer.close();
	}
	...
	public String getAuthorEmail() {
		return "antonio.cusanelli@heig-vd.ch";
	}
