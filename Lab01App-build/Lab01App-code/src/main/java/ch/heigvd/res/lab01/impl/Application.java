...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ArrayList<String> listTag = new ArrayList<>(quote.getTags());
    String quoteTxt = quote.getQuote();

    if (listTag.isEmpty() && quoteTxt.isEmpty()) {
      return;
    }

    StringBuilder dir = new StringBuilder(WORKSPACE_DIRECTORY);
    ...
    OutputStreamWriter ow = new OutputStreamWriter(new FileOutputStream(f), StandardCharsets.UTF_8);

    ow.write(quoteTxt,0,quoteTxt.length());
    ow.close();
  }
  /**
  ...
  public String getAuthorEmail() {
    return "florent.piller@heig-vd.ch";
  }
