...
public class Application implements IApplication {
  ...
  public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException {
    ...
    for (int i = 0; i < numberOfQuotes; i++) {
      ...
      //prepare filename and using method storeQuote
      String fileName = "quote-" + quote.getValue().getId() + ".utf-8";
  ...
  void storeQuote(Quote quote, String filename) throws IOException{
    ...
    //Creating folder path with quote tags
    for(String tag : quote.getTags()) {
        quotePath += File.separator + tag;
        File quoteFolders = new File(quotePath);
        quoteFolders.mkdir();
    }

    //Creating folder structure to store quotes


    quotePath += File.separator + filename;


    //Write quote into specific file
    BufferedWriter buffWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(quotePath), "UTF-8"));
  ...
  public String getAuthorEmail() {
    return "patrick.neto@heig-vd.ch";
  }
