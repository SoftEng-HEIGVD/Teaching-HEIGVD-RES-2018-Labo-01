...
public class Application implements IApplication {
    ...
    public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException {
       ...
       * quote in a text file (and for generating the directories based on the tags).
            ...
            storeQuote(quote, "quote-".concat(((Integer) i).toString()).concat(".utf8"));
    ...
    void storeQuote(Quote quote, String filename) throws IOException {
        ...
        // Getting every tag of the quote
        List<String> tags = quote.getTags();

        StringBuilder pathTags = new StringBuilder(WORKSPACE_DIRECTORY);   // Uses StringBuilder to be m or efficient
        for (String tag : tags) {

            // Adding the new directory to the path for each tag
            pathTags.append("/").append(tag);
        }

        // Finaly creating the whole arborescnce
        File tagDir = new File(pathTags.toString());
        tagDir.mkdirs();

        // We now create the quote file in the last folder
        pathTags.append("/").append(filename);
        System.out.print(pathTags.toString());
        File quoteFile = new File(pathTags.toString());

        // If the new file is successfuly created, we can write the quote inside
